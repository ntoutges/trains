...
export class TrainPart {
    constructor({ source = "missing.png", object, size = new Vector({ x: 50, y: 50 }) }) {
        this.o = object;
        this.s = source;
        this.b = size;
    ...
    updateGraphics(updateRotation = true) {
        ...
        if (updateRotation)
            this.updateRotation(-this.o.rotation);
    }
...
export class TrainParts {
    constructor({ parts = [], // part at index 0 needs to be the leader Object, and index (len-1) must be follower Object
    objects = [] }) {
        this.p = parts;
    ...
    tick(...params) {
        ...
        for (let ob of this.ob) {
            ob.tick();
        }
        // this.updateRotation();
    }
    updateGraphics(updateRotation) {
        for (const part of this.p) {
            part.updateGraphics(false);
        }
        if (updateRotation)
            this.updateRotation();
    }
    ...
    remove() {
        ...
        for (const ob of this.ob) {
            tracks.removeObject(ob);
        }
...
export class TrainAxle extends TrainPart {
    ...
    coupleTo(axle, distance = 100) {
        if (this.f2 != null)
            return;
        const followers = this.o.f; // "as" prevents errors
        const oldObject = tracks.removeObject(this.o);
        this.o = tracks.addObject(new obj.SlowFollower({
            leader: axle.o,
            distance: -distance,
            color: "#ff0000"
        }));
        ...
        for (const follower of followers) {
            if (follower.leader == oldObject)
                follower.leader = this.o;
    ...
    uncouple() {
        ...
        this.f2 = null;
        const oldObject = tracks.removeObject(this.o);
        this.o = tracks.addObject(new obj.AutoTracer({ step: 1 }));
...
export class TrainTruck extends TrainParts {
    constructor({ axles = 2, spacing = 40, object, midpointWeight = null }) {
        if (axles < 1)
            throw new Error("Invald axle count");
        const parts = [];
        const objects = [];
        parts.push(new TrainAxle({ object }));
        if (axles >= 2) {
            const followingAxles = axles - 1;
            const follower = tracks.addObject(new obj.SlowFollower({
                leader: object,
                distance: -(axles - 1) * spacing
            }));
            for (let i = 1; i < followingAxles; i++) {
                parts.push(new TrainAxle({
                    object: tracks.addObject(new obj.FloatingFollower({
                        leaderA: object,
                        leaderB: follower,
            ...
            if (midpointWeight != null) {
                objects.push(tracks.addObject(new obj.FloatingFollower({
                    leaderA: object,
                    leaderB: follower,
    ...
    getAxle(index) {
        return this.p[(index < 0) ? index + this.p.length : index];
    }
...
export class TrainBox extends TrainParts {
    constructor({ bounds = new Vector({ x: 170, y: 70 }), // x=width, y=length
    object, truckInset = 40, // distance from center of truck to edge of box
    truckMidpointWeight = 0.5, truckSpacing = 40, truckAxles = 2 }) {
        ...
        const truck2 = new TrainTruck({
            object: tracks.addObject(new obj.SlowFollower({
                leader,
                distance: 20
            })),
            axles: truckAxles,
        ...
        super({
            parts: [truck1, truck2],
            objects: [
                tracks.addObject(new obj.CenteredBox({
                    bounds,
...
//   }
// }
//# sourceMappingURL=trainParts.js.map