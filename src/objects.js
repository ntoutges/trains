...
export class Obj {
    constructor(pos = new Vector({}), color = "black") {
        this.p = pos;
        this.c = color;
    }
    getScreenCoords(offX, offY) {
        return [
            this.p.x + offX,
            -this.p.y + offY
        ];
...
export class LocomotiveTracer extends AutoTracer {
    ...
    tickO(originTrack) {
        ...
        if (super.tickO(originTrack))
            this.setVelocity(0);
        return false;
    }
}
export class FastFollower extends Obj {
    constructor({ leader, color = "blue", distance = 20 // positive indicates ahead of leader, negative indicates behind follower
     }) {
        super(new Vector({}), color);
        this.l = leader;
        this.h = null;
...
export class SlowFollower extends FastFollower {
    constructor({ leader, color = "darkgreen", distance = 20 // positive indicates ahead of leader, negative indicates behind follower
     }) {
    ...
    tick() {
        ...
        if (code) {
            ...
            for (let i = 0; i < 100; i++) { // limit to 100 rounds of correction
                ...
                if (code < 0)
                    return code / 2; // I literally have no clue why this `/ 2` needs to be here, but it works...
            }
        }
        return 1;
    }
...
export class FloatingFollower extends Obj {
    constructor({ color = "orange", leaderA = new Tracer({}), // either tracer or follower
...
export class Box extends Obj {
    constructor({ bounds = new Vector({ x: 10, y: 10 }), alpha = 0.2, color = "black" }) {
        super(new Vector({}), color);
        this.b = bounds;
        this.r = 0;
        this.a = alpha;
...
export class CenteredBox extends Box {
    constructor({ bounds = new Vector({ x: 10, y: 10 }), color = "black", alpha = 0.2, obj1 = new Obj(), obj2 = new Obj() }) {
