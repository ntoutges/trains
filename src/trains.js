...
export class RollingStock {
    ...
    coupleTo(locomotive, distance = 10) {
        ...
        if (this.cf) {
            this.uncouple(distance, true);
        }
        this.box.coupleTo(locomotive.box, distance);
        this.cf = locomotive;
        locomotive.cb = this;
    }
    uncouple(direction = 1, _override = false) {
        if (direction < 0) {
            if (!this.cb)
                return;
...
export class Locomotive extends RollingStock {
    constructor({ tracks, bounds = new math.Vector({ x: 430, y: 70 }), // x=width, y=length
    truckInset = 80, // distance from center of truck to edge of box
    truckMidpointWeight = 0.5, truckSpacing = 40, truckAxles = 2, source = "locomotive.png" }) {
        const loco = addObject(new obj.LocomotiveTracer({}));
        super({
            box: (bounds.isZero()) ?
    ...
    setRoot() {
        if (this.box.h && this.box.h[0] != this.r && this.getVelocity() != 0) {
            if (this.getVelocity() > 0) {
                ...
                this.l.l += this.box.h[0].searchTrackFor(this.r);
            } // this.l[ocomotiveTracer].l[eaderProgress]
            this.r = this.box.h[0];
        }
    }
}
export class HeadlessLocomotive extends Locomotive {
    ...
    remove() {
        ...
        if (index == -1) {
            throw new Error("HeadlessLocomotive already removed");
        }
        trainCars.splice(index, 1);
    }
}
export class Car extends RollingStock {
    constructor({ locomotive, tracks, bounds = new math.Vector({ x: 300, y: 70 }), // x=width, y=length
    truckInset = 60, // distance from center of truck to edge of box
    truckMidpointWeight = 0.5, truckSpacing = 40, truckAxles = 2, source = "wagon.png" }) {
        super({
            box: new parts.TrainBox({
                bounds,
                object: new obj.AutoTracer({ step: 0 }),
                truckInset,
                truckMidpointWeight,
                truckSpacing,
                truckAxles
        ...
        if (locomotive)
            this.coupleTo(locomotive);
        else
            this.coupleTo(new HeadlessLocomotive({ tracks })); // [Car]s cannot exist without a locomotive
    }
    coupleTo(locomotive, distance = 10) {
        if (distance < 0) { // negative distance indicates this car coupled in front (acting as locomotive)
            return locomotive.coupleTo(this, -distance);
        }
        if (this.cf && this.cf instanceof HeadlessLocomotive)
            this.cf.remove();
        super.coupleTo(locomotive, distance);
    }
    uncouple(direction = 1, override = false) {
        if (direction < 0) {
            if (!this.cb)
                return;
            this.cb.uncouple(-direction);
...
export class FastLocomotive extends Locomotive {
    constructor({ tracks, bounds = new math.Vector({ x: 430, y: 70 }), // x=width, y=length
    truckInset = 80, // distance from center of truck to edge of box
    truckMidpointWeight = 0.7, truckSpacing = 40, truckAxles = 3, source = "locomotive.png", keys = {
        ...
        if (!(0 in keys))
            keys[0] = ["accelerateTo", 0, 0.005]; // specifies deceleration
        this.keys = keys;
    ...
    onKeyDown(keyCode) { if (keyCode in this.keys)
        this[this.keys[keyCode][0]].apply(this, this.keys[keyCode].slice(1)); }
    onKeyUp(keyCode) { if (keyCode in this.keys && this.keys[keyCode][0] == "accelerateTo") {
        this[this.keys[0][0]].apply(this, this.keys[0].slice(1));
    } }
}
